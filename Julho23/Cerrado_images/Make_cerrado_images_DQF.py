# -*- coding: utf-8 -*-
"""
Created on Wed Jul 12 15:52:10 2023
...
def get_lat_lon(file_system):
    files = file_system.ls('noaa-goes16/ABI-L2-FDCF/2020/'+str(200).zfill(3)+'/'+str(15).zfill(2)+'/') # list of 6 files for 2022 day 200, UTC time 15:00, 15:10, 15:20, ..., 15:50
...
for d in range(210,211):
    ...
    # while i<len(files): #len(files):
        for i in range(1,len(files)):
            ...
            # Z=varlist[0]
            im = ax.imshow(M_box,transform=oproj, cmap=cmaplist[0], norm=normlist[0],vmin=minlist[0],vmax=maxlist[0])
            ...
            cb = fig.colorbar(im,cax=cax, ticks=ticklist[0] ,orientation="vertical",shrink=0.7,pad=0.02)
            ...
            # plt.plot()
            # plt.show()
            plt.savefig('DQF_cerrado_small_area_'+plottitle+'.png', dpi = 100)
            # camera.snap()
            # fig.clear()
