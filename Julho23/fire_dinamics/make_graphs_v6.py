...
data = sorted(glob.glob(datadir+'/*smold_v4.csv'))
...
# aux1 ='sat,year,julian,hhmm,code,central_lat,central_lon,sum_FRP(MW),mean_FRP,N_FRP,Temp,N_temp,N_smold,N_flame\n'
aux1 ='sat,year,julian,hhmm,code,central_lat,central_lon,sum_FRP(MW),mean_FRP,N_FRP,Temp,N_temp,N_smold,N_flame\n'
...
##########################################################################################################
    ...
    return
def mean_FRPx_mean_Temp(dados_emission,filename,matrix):
    header = list(dados_emission)
    ...
    pos4=header.index('mean_FRP')
    column4=dados_emission.iloc[:,pos4]
    mean_FRP = np.array(column4.dropna(), dtype=float)
    
    pos5=header.index('Temp')
    column5=dados_emission.iloc[:,pos5]
    Temp = np.array(column5.dropna(), dtype=float)
    ...
    pyplot.clf()
    pyplot.title(filename)
    # pyplot.xlabel('mean_Temp(K)')
    # pyplot.ylabel('mean_FRP(MW)')
    pyplot.xlabel('log(mean_Temp(K))')
    pyplot.ylabel('log(mean_FRP(MW))')
    # 7.75,-55.25
    # index = np.where((lat==-7.75) & (lon==-55.25))
    # code_full = code[index]
    # N_flame_index = N_flame[index]
    ...
    index = np.where((lat==matrix[0,0]) & (lon==matrix[0,1]) & (mean_FRP>=0) & (Temp>=0) )
    # lon_index = np.where(lon==-56.75)
    # print(index)
    Temp_index = Temp[index]
    FRP_index = mean_FRP[index]

    for i in range(1,len(matrix)):
        index = np.where((lat==matrix[i,0]) & (lon==matrix[i,1]) & (mean_FRP>=0) & (Temp>=0))
        # print(index)
        Temp_index_aux = Temp[index]
        FRP_index_aux = mean_FRP[index]
        
        Temp_index = np.append(Temp_index,Temp_index_aux)
        FRP_index = np.append(FRP_index,FRP_index_aux)
    # fire_duration = np.append(fire_duration,fire_duration_partial)


    # pyplot.plot(Temp_index,FRP_index,'*', markersize=3)
    # pyplot.yscale("log")
    # pyplot.xscale("log")
    print(type(Temp_index))
    
    # constante_positiva = 10e-10
    Temp_index = np.log(Temp_index)
    FRP_index = np.log(FRP_index)
    pyplot.hist2d(Temp_index, FRP_index,bins=80, cmap='viridis')
    pyplot.colorbar()
    pyplot.savefig(filename+'.png',dpi=200)
...
# fire_durations,position_peaks,position_peaks_start,position_peak_end = get_data(M)
# fire_duration_graph(fire_durations,'Fire_durations_histogram_2021_2022_full_v5')
# hist_start_to_peak_graph(position_peaks,position_peaks_start,'Start_to_peak_histogram_2021_2022_full_v5')
# hist_peak_to_end_graph(position_peaks,position_peaks_start,'Peak_to_end_histogram_2021_2022_full_v5')
###############################################################################
...
# FRP_stats_v5(dados_emission, 'Plot_FRP_quarter_area_small_box_amazon_full')
mean_FRPx_mean_Temp(dados_emission, 'Plot_log_FRPxTemp_quarter_area_small_box_amazon_v2',M)
